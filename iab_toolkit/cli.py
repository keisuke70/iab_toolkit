...
def cmd_classify_text(args):
    """Handle direct text classification command."""
    try:
        # Import the classification functions
        from iab_toolkit.classify import classify_with_embeddings
        from iab_toolkit._gpt import classify_with_gpt, build_persona_tags
        ...
        # Step 2: GPT-based refinement (optional)
        categories_final = []
        if not args.embedding_only and categories_embedding:
            try:
                categories_final = classify_with_gpt(
                    text_content, 
                    max_categories=args.max_categories
                )
            except Exception as e:
                print(f"GPT classification failed: {e}")
                categories_final = categories_embedding
        else:
            categories_final = categories_embedding
        
        # Step 3: Persona generation (optional)
        persona = None
        if not args.no_persona:
            try:
                persona = build_persona_tags(text_content)
            except Exception as e:
        ...
        if args.json:
            ...
            json_result = {
                ...
                'categories_embedding': [
                    ...
                    for cat in categories_embedding
                ],
                'categories_final': [
                    {
                        'id': cat.id,
                ...
                'persona': {
                    ...
                    'gender_tilt': persona.gender_tilt,
            ...
            print(format_categories_output(categories_embedding, "Embedding Categories"))
            print()
            
            # Show final results
            if categories_final != categories_embedding and not args.embedding_only:
                print(format_categories_output(categories_final, "Final Categories (after GPT)"))
            elif args.embedding_only:
                print("Final Categories: Using embedding results only")
            else:
                print("Final Categories: Same as embedding results")
            
            # Show Tier 2 categories specifically (client's request)
            tier2_categories = [cat for cat in categories_final if cat.tier_2]
            if tier2_categories:
                print(f"\n{'='*30}")
                print("TIER 2 CATEGORIES (Client Focus)")
                print("="*30)
                for i, cat in enumerate(tier2_categories, 1):
                    print(f"{i}. {cat.tier_2} (Score: {cat.score:.3f})")
            
            # Show persona
            if not args.no_persona and persona:
                print()
...
def main():
    ...
    # First, try to detect if we're using old-style syntax (URL as first argument)
    import sys
    if len(sys.argv) > 1 and sys.argv[1].startswith(('http://', 'https://')):
        # Old-style syntax: python -m iab_toolkit.cli https://example.com
        ...
        # New-style syntax with subcommands
        ...
        text_classify_parser = subparsers.add_parser('classify-text', help='Classify text directly')
        text_classify_parser.add_argument("text", nargs='?', help="Text to classify (optional, use -f/--file to specify a file)")
        text_classify_parser.add_argument("-f", "--file", dest="text_file", help="File containing text to classify")
